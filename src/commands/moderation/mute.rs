...
pub async fn mute(
    ...
    if user.user.bot() {
        let error_msg = locales.get("commands.moderation.mute.error_user_bot", guild_lang, &[]).await;
        send_reply(ctx, CreateReply::new().content(error_msg).ephemeral(true)).await?;
        return Ok(());
    }

    if user.user.id == ctx.author().id {
        let error_msg = locales.get("commands.moderation.mute.error_user_self", guild_lang, &[]).await;
        send_reply(ctx, CreateReply::new().content(error_msg).ephemeral(true)).await?;
        return Ok(());
    }
    
    if user.user.id == ctx.guild().unwrap().owner_id {
        let error_msg = locales.get("commands.moderation.mute.error_user_owner", guild_lang, &[]).await;
        send_reply(ctx, CreateReply::new().content(error_msg).ephemeral(true)).await?;
        return Ok(());
    }

    if user.permissions(ctx.cache()).unwrap().administrator() {
        let error_msg = locales.get("commands.moderation.mute.error_user_admin", guild_lang, &[]).await;
        send_reply(ctx, CreateReply::new().content(error_msg).ephemeral(true)).await?;
        return Ok(());
    }
    ...
    if user_highest_role_position >= bot_highest_role_position {
        let error_msg = locales.get("commands.moderation.mute.error_user_higher_role", guild_lang, &[]).await;
    ...
    if parse_to_time(duration.clone()).is_none() {
        let error_msg = locales.get("commands.moderation.mute.error_invalid_time", guild_lang, &[]).await;
        send_reply(ctx, CreateReply::new().content(error_msg).ephemeral(true)).await?;
        return Ok(());
    }
    ...
    if date.timestamp() - Utc::now().timestamp() > 2419200 {
        let error_msg = locales.get("commands.moderation.mute.error_time_too_long", guild_lang, &[]).await;
        send_reply(ctx, CreateReply::new().content(error_msg).ephemeral(true)).await?;
        return Ok(());
    }
    ...
    if  is_muted {
        let error_msg = locales.get("commands.moderation.mute.error_user_already_muted", guild_lang, &[]).await;
        send_reply(ctx, CreateReply::new().content(error_msg).ephemeral(true)).await?;
        return Ok(());
    }
    ...
    let no_reason = locales.get("commands.moderation.mute.no_reason", guild_lang, &[]).await;
    ...
    let content = locales.get("commands.moderation.mute.reply_success", guild_lang, &[
        ...
        TranslationParam::String(action_reason.as_deref().unwrap_or(&*no_reason).to_string())
    ]).await;
